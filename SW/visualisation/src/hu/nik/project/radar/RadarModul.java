...
public class RadarModul implements IRadarSensor, ICommBusDevice {
    ...
    public RadarModul(ISensorScene sensorScene,CommBus commbus, float angelOfSight, int samplingTime,double currentSpeed,Vector2D currentPos ) {
        ...
        _radarPackets = new ArrayList<RadarPacket>();
    ...
    * */
    ...
    public ObservableList<RadarPacket> getDetectedObjsRelativeSpeedAndDistance(ArrayList<Vector2D> incomingData) {
    ...
    public RadarPacket getDetectedObjRelativeSpeedAndDistance() {
    ...
    private ObservableList<RadarPacket> getDetectedObjsRelativeSpeedDistance(ArrayList<Vector2D> recentVectorList, Vector2D ourCurrentPosition, double ourSpeed) {
        if (_radarPackets != null && _radarPackets.size() != 0) {
            if (recentVectorList.size() != 0) {
                ...
                while (itemIndex != recentVectorList.size()) {
                    ...
                    RadarPacket radarPacket = IsVectorInSpeedandDistObjList(currentVector);
                    if (radarPacket != null) {
                        ...
                        RadarPacket newSpeedDistObj = new RadarPacket(0, 0, currentVector);
                ...
                while (myListIterator.hasNext()) {
                    RadarPacket removableObj = ((RadarPacket) myListIterator.next());
                    myListIterator.remove();
                }
            } else {
                _radarPacketObservableList = FXCollections.observableList(new ArrayList<RadarPacket>());
                return _radarPacketObservableList;
            }
        } else {
            if (recentVectorList.size() != 0) {
                for (int i = 0; i < recentVectorList.size(); i++) {
                    ...
                    RadarPacket radarPacket = new RadarPacket(0, 0, item);
    ...
    private double getCurrentSpeedOfSpecificObj(RadarPacket previous, Vector2D vector2D) {
    ...
    private RadarPacket IsVectorInSpeedandDistObjList(Vector2D vector) {
        int index = 0;
    ...
    private RadarPacket getClosestObjectFromList(ObservableList<RadarPacket> radarPacketList) {
        int min = 0;
...
//</editor-fold>
    ...
    public interface OnRadarObjectsListener {
        void objectListChanged(ObservableList<RadarPacket> result);
    }
